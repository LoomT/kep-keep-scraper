-- Generated primary keys (person_id, organisation_id, comment_id start at their project_id*1_000_000)
-- 1. Project Table
...
(
    ...
    domain    TEXT, -- can be ‘email’ domain eg “github.com”, “email”
    username  TEXT, --  e.g. “stijn@tudelft.nl”
...
(
    ...
    normalised_status TEXT     NOT NULL CHECK (normalised_status IN
                                               ('accepted', 'rejected', 'draft', 'review', 'withdrawn',
                                                'unknown')), -- superseded -> rejected, null or not clear -> unknown
...
(
    project_id     INTEGER,
    proposal_id    TEXT,
    revision_index INTEGER, -- multiple rows with the same project_id, proposal_id and revision_index but different author_ids means multiple authors at some revision
    author_id      INTEGER,
    PRIMARY KEY (
                 project_id,
                 proposal_id,
...
(
    ...
    comment_on_comment_id INTEGER, -- id of parent comment, null if root comment (discussion, issue, pr, etc.)
    created_at            DATETIME,
